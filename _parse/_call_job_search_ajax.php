...
$job_query = mysqli_query($db_connection, $select_query) or die (mysqli_error($db_connection, $mysqli_error));
$rjoblist="";
...
while($row = mysqli_fetch_array($job_query)){
	...
	while ($row = mysqli_fetch_array($_query, MYSQLI_ASSOC)) {
		...
		while ($row = mysqli_fetch_array($query1, MYSQLI_ASSOC)) {
			...
			if($is_active == 0){
				$rjoblist .= '<div id="jb_'.$job_id.'" class="job jb_'.$job_id.'"><div class="job-user-image hand" onclick="admins(\''.$comp_id.'\')" style="vertical-align: inherit;">'.$company_pic.'</div>';
				$rjoblist .= '<div class="job-details"><h3><a href="javascript:void(0)" onclick="admins(\''.$comp_id.'\')"><h3>'.$admin_name.'</h3></a></h3>';
				...
				$rjoblist .= '<div id="jb_'.$job_id.'" class="job jb_'.$job_id.'" style="background-color: #fee3e3;"><div class="job-user-image hand" onclick="admins(\''.$comp_id.'\')" style="vertical-align: inherit;">'.$company_pic.'</div>';
				$rjoblist .= '<div class="job-details"><h3><a href="javascript:void(0)" onclick="admins(\''.$comp_id.'\')"><h3>'.$admin_name.'</h3></a></h3>';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Job title:</span> '.$job_title.'</h4> ';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Job type:</span> '.$job_type.'</h4> ';
				$rjoblist .= '<h4 style="margin-top:5px;"><span class="grey-out">Posted on:</span> '.$created_date.'</h4> ';
