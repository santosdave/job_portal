...
if (isset($_POST['action']) && $_POST['action'] == "add_xp"){
	...
	if($from_year > $to_year){
		...
		if($ps_xp_num > 5){
			...
			$sql = "INSERT INTO experience_detail (e_hash ,job_title ,job_specialization , admin_name, business_stream, start_date, date_date, date_year_diff, description, postdate) VALUES ('$log_email','$job_nm','$job_spec','$job_company','$bb_stream','$from_xp_date','$to_xp_date','$date_year_diff','$description',now())";
			$query = mysqli_query($db_connection, $sql);
			...
			while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
				...
				$xpList .= '<div class="profile_widget-details"><h3>'.$job_title.'</h3><h4 style="margin-top:5px;">'.$admin_name.' - '.$business_stream.' </h4>';
...
if (isset($_POST['action']) && $_POST['action'] == "cn_update"){
	...
	$cname = $_POST['cname'];		
	$urle = $_POST['url'];		
	$sql1 = "UPDATE admin_profile SET admin_name='$cname', company_website_url='$urle' WHERE e_hash='$log_email' LIMIT 1";
	$query = mysqli_query($db_connection, $sql1);
	echo $cname;
	exit();
}
